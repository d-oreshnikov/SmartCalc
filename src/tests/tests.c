...
START_TEST(ln_cos) {
    ...
    double answer = 0;
    int status = calculate_equation(equation, PI, &answer);
    double result = 0.0;
...
START_TEST(ln_sin) {
    ...
    double answer = 0;
    int status = calculate_equation(equation, PI / 2, &answer);
    double result = 0;
    ck_assert_double_eq_tol(result, answer, 1e-7);
...
START_TEST(sin_sqrt_4) {
    ...
    double answer = 0;
    int status = calculate_equation(equation, DUMMY_DOUBLE, &answer);
...
START_TEST(degree_log) {
    char *equation = "log(2.5 ^ 2.5)";
    double answer = 0;
    int status = calculate_equation(equation, DUMMY_DOUBLE, &answer);
    double result = 0.99485;
    ck_assert_double_eq_tol(result, answer, 1e-1);
    ck_assert_int_eq(status, SUCCESS);
}
...
int main() {
    ...
    tcase_add_test(tc1_1, degree_log);
